...
fn main() {
    let arg_match = App::new("sshTool").version("1.0")
        ...
        .subcommand(SubCommand::with_name("add").about("添加服务器")
            ...
            .arg(Arg::with_name("private_key").long("key").short("k").value_name("STRING").help("SSH秘钥"))
            .arg(Arg::with_name("identity_file").long("identity").short("i").value_name("STRING").help("服务器秘钥"))
        )
